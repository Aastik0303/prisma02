    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-white grid place-items-center">
      <section className="w-full max-w-md rounded-3xl border border-indigo-400/20 bg-slate-900 p-7 shadow-2xl">
        ...
        <form onSubmit={submit} className="mt-7 space-y-4">
          ...
          {error && <p className="rounded-xl bg-rose-500/10 px-4 py-3 text-xs font-bold text-rose-300">{error}</p>}
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-white">
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.25em] text-indigo-400">Private developer portal</p><h1 className="mt-2 text-3xl font-black">User analytics</h1></div><button onClick={() => { sessionStorage.removeItem(TOKEN_KEY); window.location.assign('/'); }} className="flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2 text-xs font-bold"><LogOut className="h-4 w-4" /> Sign out</button></header>
